...
export function TrustLeaderboardPage() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-b from-slate-50 via-slate-100/60 to-white px-3 pb-10 pt-3 sm:px-5 md:px-6">
      <div className="pointer-events-none absolute -left-20 top-8 h-56 w-56 rounded-full bg-violet-300/25 blur-3xl" />
      <div className="pointer-events-none absolute right-0 top-0 h-72 w-72 rounded-full bg-indigo-300/15 blur-3xl" />

      <div className="relative mx-auto w-full max-w-5xl">
        <section className="rounded-3xl border border-indigo-100 bg-[linear-gradient(135deg,#faf9ff_0%,#ede9fe_52%,#f8f7ff_100%)] px-5 py-7 shadow-[0_35px_90px_-65px_rgba(67,56,202,0.45)] sm:px-8 sm:py-9">
          <p className="inline-flex items-center gap-2 rounded-full border border-violet-200 bg-white px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-violet-700">
        ...
        {myRank ? (
          <section className="mt-4 rounded-2xl border border-slate-200 bg-white/90 p-4 shadow-sm">
            ...
            <div className="mt-2 flex items-center justify-between gap-3">
              ...
              <p className="text-sm font-semibold text-violet-700">{myRank.trustPoints || 0} pts</p>
            </div>
          </section>
        ) : null}
                ...
                return (
                  ...
                  >
                    <div className="flex min-w-0 items-center gap-3">
                      <span className={`inline-flex h-8 w-8 items-center justify-center rounded-full text-sm font-bold ${isTopThree ? "bg-violet-600 text-white" : "bg-slate-100 text-slate-700"}`}>
                        {user.rank}
                      </span>
